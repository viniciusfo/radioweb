...
static VS1053_FilePlayer *myself;
...
boolean VS1053_FilePlayer::useInterrupt(uint8_t type) {
  myself = this;  // oy vey
...
VS1053_FilePlayer::VS1053_FilePlayer(
	       int8_t rst, int8_t cs, int8_t dcs, int8_t dreq, 
	       int8_t cardcs) 
               : VS1053(rst, cs, dcs, dreq) {
...
VS1053_FilePlayer::VS1053_FilePlayer(
	       ...
	       int8_t rst, int8_t cs, int8_t dcs, int8_t dreq, 
	       int8_t cardcs) 
               : VS1053(mosi, miso, clk, rst, cs, dcs, dreq) {
...
boolean VS1053_FilePlayer::playFullFile(const char *trackname) {
...
void VS1053_FilePlayer::stopPlaying(void) {
  // cancel all playback
...
void VS1053_FilePlayer::pausePlaying(boolean pause) {
  if (pause) 
    playingMusic = false;
  else {
    playingMusic = true;
...
boolean VS1053_FilePlayer::paused(void) {
  return (!playingMusic && currentTrack);
}

boolean VS1053_FilePlayer::stopped(void) {
  return (!playingMusic && !currentTrack);
}


boolean VS1053_FilePlayer::startPlayingFile(const char *trackname) {
  // reset playback
...
void VS1053_FilePlayer::feedBuffer(void) {
  noInterrupts();
...
void VS1053_FilePlayer::feedBuffer_noLock(void) {
...
VS1053::VS1053(int8_t mosi, int8_t miso, int8_t clk, 
			   int8_t rst, int8_t cs, int8_t dcs, int8_t dreq) {
  _mosi = mosi;
  _miso = miso;
  _clk = clk;
...
VS1053::VS1053(int8_t rst, int8_t cs, int8_t dcs, int8_t dreq) {
  _mosi = 0;
  _miso = 0;
  _clk = 0;
  useHardwareSPI = true;
...
void VS1053::applyPatch(const uint16_t *patch, uint16_t patchsize) {
  uint16_t i = 0;
...
boolean VS1053::prepareRecordOgg(char *plugname) {
...
void VS1053::startRecordOgg(boolean mic) {
...
void VS1053::GPIO_pinMode(uint8_t i, uint8_t dir) {
  if (i > 7) return;
...
void VS1053::GPIO_digitalWrite(uint8_t val) {
  sciWrite(VS1053_REG_WRAMADDR, VS1053_GPIO_ODATA);
  sciWrite(VS1053_REG_WRAM, val);
}

void VS1053::GPIO_digitalWrite(uint8_t i, uint8_t val) {
  if (i > 7) return;
